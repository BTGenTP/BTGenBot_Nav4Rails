...
from nav4rail_graph_rag.observability import NullTracker, WandbTracker
...
class GraphRAGPipeline:
    ...
    def run(self, mission: str | Mission, top_k: int = 5) -> GenerationResult:
        ...
        result = GenerationResult(mission_obj, steps, xml, validation, retrieval)
        self.tracker.log_generation(result)
        metadata = getattr(self.llm, "last_metadata", None)
        if metadata is not None:
            self.tracker.log_metrics(
                {
                    "llm_latency_s": metadata.latency_s,
                    "llm_prompt_tokens": metadata.prompt_tokens,
                    "llm_completion_tokens": metadata.completion_tokens,
                    "llm_total_tokens": metadata.total_tokens,
                    "llm_cost_usd": metadata.cost_usd,
                },
                prefix="generation",
            )
        return result
...
def build_pipeline(records, nodes=(), edges=(), patterns=(), llm=None, tracker=None) -> GraphRAGPipeline:
    ...
    return GraphRAGPipeline(HybridRetriever(lexical, graph), llm=llm, tracker=tracker)
